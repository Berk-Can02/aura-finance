...
export function CreateGroupDialog({ onCreateGroup }: CreateGroupDialogProps) {
  ...
  const handleRemoveMember = (email: string) => setMemberEmails(memberEmails.filter(e => e !== email));
  ...
  const handleSubmit = () => {
    ...
    setName(""); setDescription(""); setMemberEmails([]); setOpen(false);
  };
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4 py-4">
          <div className="space-y-2">
            ...
            <Input id="name" placeholder={t("groupsPage.groupNamePlaceholder")} value={name} onChange={(e) => setName(e.target.value)} maxLength={50} />
          </div>

          <div className="space-y-2">
            <Label htmlFor="description">{t("groupsPage.descriptionOptional")}</Label>
            <Textarea id="description" placeholder={t("groupsPage.descriptionPlaceholder")} value={description} onChange={(e) => setDescription(e.target.value)} maxLength={200} rows={2} />
          </div>

          <div className="space-y-2">
            <Label htmlFor="members">{t("groupsPage.addMembers")}</Label>
            <div className="flex gap-2">
              <Input id="members" placeholder={t("groupsPage.memberEmailPlaceholder")} type="email" value={memberEmail}
              ...
              <div className="flex flex-wrap gap-2 pt-2">
                {memberEmails.map((email) => (
                  <Badge key={email} variant="secondary" className="pl-2 pr-1 py-1">
                    {email}
                    <button onClick={() => handleRemoveMember(email)} className="ml-1 p-0.5 rounded-full hover:bg-muted-foreground/20">
                      <X className="w-3 h-3" />
                    </button>
                  </Badge>
                ))}
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSubmit} className="gradient-primary">{t("groupsPage.createGroup")}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
